...
class Interface
  @@create = {1 => :make_station, 2 => :make_route, 3 => :make_train, 4 => :make_wagon, 5 => :print_menu_create}
  @@operations = {1 => :station_to_route, 2 => :wagon_to_train, 3 => :route_to_train, 4 => :move_train, 5 => :volume_to_wagon, 6 => :print_menu_operations}
  @@info = {1 => :stations_list, 2 => :routs_list, 3 => :trains_list, 4 => :wagons_list, 5 => :print_menu_info}
...
# Печать информации ------------------------------------------------------------------------
  ...
  def route_info(route)
    ...
    if route.stations.size > i
      route.stations[1..(route.stations.size - i)].each.with_index(1) {|station, index| puts "Станция № #{index} - '#{station.name}'."}
    end
    i =  1 - i
...
# Функции - действия с объектами -------------------------------------------------------------------------
  ...
  def station_to_route
    ...
    when 1
      ...
      k = route.ring? ? route.stations.size : route.stations.size - 1
